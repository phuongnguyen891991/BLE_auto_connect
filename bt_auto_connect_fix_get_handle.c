 ...
 *  Foundation, Inc., 51 F anklin St, Fifth Floor, Boston, MA  02110-1301  USA
 *
 */
...
#define FLAGS_WR_CMD				0x02
#define FLAGS_RESERVED				0x03
...
static int print_advertising_devices(int dd, uint8_t filter_type)
{
	...
	while ((len = read(dd, buf, sizeof(buf))) < 0)
		...
		if (check_report_filter(filter_type, info)) 
		{
			...
			{
				if(le_devices.status == DEV_UNCONFIGURED ||
					le_devices.status == DEV_CONFIGURED)
				{
...
static gboolean char_write_auto(gpointer user_data)
{
	...
	gatt_write_char(attrib, opt_handle, value, len, 
								char_write_req_cb,NULL);
	return FALSE ;
error:
	flags_connect = FLAGS_RESERVED;
	g_main_loop_quit(event_loop);
	return FALSE ;
}
							...
							gpointer user_data )
{
	...
	for (l = characteristics; l; l = l->next) 
	{
		...
		strncpy(string2uuid,chars->uuid+4,4);
